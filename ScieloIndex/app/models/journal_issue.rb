class JournalIssue < ActiveRecord::Base
  ...
  validates_length_of :number, :volume, :supplement, :maximum => 100, :allow_nil => true
  validates_length_of :title, :maximum => 200, :allow_nil => true
  validates_format_of :number, :volume, :with => /^[-a-zA-Z0-9áéíóúÁÉÍÓÚñÑ:,.& ]*$/
  ...
  validates_uniqueness_of :journal_id, :scope => [:number, :volume, :year, :supplement]
